...
function computeParticlesPreference(): boolean {
...
export function ParticlesBackground() {
  const [enabled, setEnabled] = useState(() => computeParticlesPreference())

  useEffect(() => {
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)')
    const narrow = window.matchMedia('(max-width: 740px)')
    const sync = () => setEnabled(computeParticlesPreference())
    sync()
